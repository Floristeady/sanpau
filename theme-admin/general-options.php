...
/*	1)	Add "Boilerplate Admin" link to left-nav Admin Menu & callback function for clicking that menu link */
	...
	if ( ! function_exists( 'create_boilerplate_admin_page' ) ):
		function create_boilerplate_admin_page() {
			add_theme_page( __('General Options', 'sanpau'), __( 'General Options', 'sanpau'), 'administrator', 'boilerplate-admin', 'build_boilerplate_admin_page');
		}
	...
	if ( ! function_exists( 'build_boilerplate_admin_page' ) ):
		...
		?>
			<div id="boilerplate-options-wrap">
				...
				<h2><?php _e( 'General options for', 'sanpau' ); ?> <?php echo wp_get_theme() . __( '', 'sanpau' );?> </h2>
				<p><?php _e( 'This is the web site manager. Select the options you want to include on the website.', 'sanpau' ); ?></p>
...
/*	3)	Add "Boilerplate Admin" Page options */
	...
	if ( ! function_exists( 'google_verification_setting' ) ):
		function google_verification_setting() {
			...
			echo _e( '<p>Add <a href="https://support.google.com/webmasters/answer/35179">Google Verification</a> code and <a href="https://www.google.com/analytics">Google Universal Analytics </a> code to the <code>&lt;head&gt;</code> of all your pages.</p>', 'sanpau' );
			echo _e( '<p>To include Google Analytics and Google Verification code, select this option and add your Verification number here and the site domain:</p>', 'sanpau' );
			echo _e( '<p>Google Analytics ID: </p>', 'sanpau' );
			...
			echo _e( '<p>This will add the following code to the <code>&lt;head&gt;</code> of your pages, where <code>XXXXXXXXX?</code> will be replaced with the code you insert above.</p><br/>', 'sanpau' );
			
			echo _e( '<p>Site Domain: </p>', 'sanpau' );
			...
			echo _e( '<p>And will add the latest version of the Analytics tracking code, to track website visitors with Google Analytics.</p>', 'sanpau' );
		}
	...
	if ( ! function_exists( 'favicon_setting' ) ):
		function favicon_setting() {
			...
			echo _e( '<p>If you plan to use a <a href="http://en.wikipedia.org/wiki/Favicon">favicon</a> for your site, place the "favicon.ico" file in the root directory of your site.</p>', 'sanpau' );
			echo _e( '<p>If the file is in the right location, you don\'t really need to select this option, browsers will automatically look there and no additional code will be added to your pages.</p>', 'sanpau' );
			echo _e( '<p>Selecting this option will add the following code to the <code>&lt;head&gt;</code> of your pages:</p>', 'sanpau' );
	...
	if ( ! function_exists( 'modernizr_js_setting' ) ):
		function modernizr_js_setting() {
			...
			echo _e('<p><a href="http://modernizr.com">Modernizr</a> is a JS library that appends classes to the <code>&lt;html&gt;</code> that indicate whether the user\'s browser is capable of handling advanced CSS, like "cssreflections" or "no-cssreflections".  It\'s a really handy way to apply varying CSS techniques, depending on the user\'s browser\'s abilities, without resorting to CSS hacks.</p>', 'sanpau' );
			echo _e('<p>Selecting this option will add the following code to the <code>&lt;head&gt;</code> of your pages (note the lack of a version, when you\'re ready to upgrade, simply copy and paste the new version into the file below, and your site is ready to go!):</p>', 'sanpau' );
			echo '<code>&lt;script src="' .BP_THEME_URL. '/js/modernizr.js"&gt;&lt;/script&gt;</code>';
			echo _e('<p><strong>Note: If you do <em>not</em> include Modernizr, the IEShiv JS <em>will</em> be added to accommodate the HTML5 elements used in Boilerplate in weaker browsers:</strong></p>', 'sanpau' );
	...
	if ( ! function_exists( 'jquery_js_setting' ) ):
		function jquery_js_setting() {
			...
			echo _e('<p><a href="http://jquery.com/">jQuery</a> is a JS library that aids greatly in developing high-quality JavaScript quickly and efficiently.</p>', 'sanpau' );
			echo  _e( '<p>Selecting this option will add the following code to your pages just before the: <code>&lt;/body&gt;</code></p>', 'sanpau' );
			...
			echo _e('<p><strong>Note: <a href="http://developer.yahoo.com/blogs/ydn/posts/2007/07/high_performanc_5/">Best-practices</a> recommend that you load JS as close to the <code>&lt;/body&gt;</code> as possible.  If for some reason you would prefer jQuery and jQuery plug-ins to be in the <code>&lt;head&gt;</code>, please select this option.</strong></p>', 'sanpau' );
			echo _e('<p>The above code first tries to download jQuery from Google\'s CDN (which might be available via the user\'s browser cache).  If this is not successful, it uses the theme\'s version.</p>', 'sanpau' );
			echo _e('<p><strong>Note: This plug-in tries to keep current with the most recent version of jQuery.  If for some reason you would prefer to use another version, please indicate that version:</strong></p><br/>', 'sanpau' );
	...
	if ( ! function_exists( 'site_js_setting' ) ):
		function site_js_setting() {
			...
			echo _e( '<p>If you would like to add your own site JavaScript file, we provides a starter file located in:</p>', 'sanpau' );
			echo '<code>' .BP_THEME_URL. '/js/script-starter.js</code>';
			echo _e( '<p>Add what you want to that file and select this option.</p>', 'sanpau' );
			echo  _e( '<p>Selecting this option will add the following code to your pages just before the: <code>&lt;/body&gt;</code></p>', 'sanpau' );
	...
	if ( ! function_exists( 'cache_buster_setting' ) ):
		function cache_buster_setting() {
			...
			echo _e( '<p>To force browsers to fetch a new version of a file, versus one it might already have cached, you can add a "cache buster" to the end of your CSS and JS files.</p>', 'sanpau' );
			echo _e( '<p>To increment the cache buster version number, type something here:</p><br />', 'sanpau' );
			echo '<input type="text" size="4" name="plugin_options[cache_buster_version]" value="'.$version.'"></p>';
			echo _e( '<p>Selecting this option will add the following code to the end of all of your CSS and JS file names on all of your pages:</p>', 'sanpau' );
	...
	if ( ! function_exists( 'meta_tags_setting' ) ):
		function meta_tags_setting() {
			...
			echo _e( '<p>Add meta keywords tags for SEO, write your keywords separated by a comma, but not a space.</p>', 'sanpau' );
			...
			echo _e('<p>Selecting this option will add the following code to the <code>&lt;head&gt;</code> of your pages, where </code>tags...</code> will be replaced with the code you insert above</p>', 'sanpau' );
